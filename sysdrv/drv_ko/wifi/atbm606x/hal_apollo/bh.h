...
enum atbm_rx_frame_type{
	...
	ATBM_RX_WSM_8023_DATA_FRAME,
	ATBM_RX_WSM_GRO_FLUSH,
};
#include "bh_usb.h"
...
static  inline void atbm_ieee80211_rx_8023(struct ieee80211_hw *hw,struct ieee80211_sta *pubsta,struct sk_buff *skb)
{
	skb->pkt_type = 0;
	
	if(softirq_count() == 0){
		local_bh_disable();
		ieee80211_rx_8023(hw,pubsta,skb);
		local_bh_enable();	
	}else  {
		ieee80211_rx_8023(hw,pubsta,skb);
	}
