					 ...
					 u16 agg_size, u16 timeout)
{
	...
	info->ignore_reason = IEEE80211_QUEUE_STOP_REASON_AGGREGATION_PENDING;
...
static void ieee80211_tx_ba_wake_queue(struct sta_info *sta,struct tid_ampdu_tx *tid_tx,u16 tid)
{
	if(tid_tx == NULL){
		return;
	}
	
	if (test_and_clear_bit(HT_AGG_STATE_QUEUE_STOP, &tid_tx->state)){
		int queue = sta->sdata->vif.hw_queue[ieee80211_ac_from_tid(tid)];
		ieee80211_wake_queue_by_reason(
			&sta->local->hw, queue,
			IEEE80211_QUEUE_STOP_REASON_AGGREGATION_PENDING);
		atbm_printk_err("aggtx wake queue\n");
	}
}

static void ieee80211_tx_ba_stop_queue(struct sta_info *sta,struct tid_ampdu_tx *tid_tx,u16 tid)
{
	if(tid_tx == NULL){
		return;
	}

	if (WARN_ON(test_and_set_bit(HT_AGG_STATE_QUEUE_STOP, &tid_tx->state))){
		
	}else {
		int queue = sta->sdata->vif.hw_queue[ieee80211_ac_from_tid(tid)];
		ieee80211_stop_queue_by_reason(
			&sta->sdata->local->hw, queue,
			IEEE80211_QUEUE_STOP_REASON_AGGREGATION_PENDING);
		atbm_printk_err("aggtx stop queue\n");
	}
}
				    ...
				    bool tx)
{
	...
	if(sta->ampdu_mlme.tid_start_tx[tid]){
		tid_tx = sta->ampdu_mlme.tid_start_tx[tid];
		sta->ampdu_mlme.tid_start_tx[tid] = NULL;
		ieee80211_tx_ba_wake_queue(sta,tid_tx,tid);
#if ((LINUX_VERSION_CODE < KERNEL_VERSION(2,6,40)) || (defined (ATBM_ALLOC_MEM_DEBUG)))
...
//		       pubsta->addr);
	...
	ieee80211_tx_ba_stop_queue(sta,tid_tx,tid);
